...
class GlobalsTableModel(BinsyncTableModel):
    """Model for the Globals (variables) tab. Rowdata is [addr, name]; checkbox and id at col 0."""
    ...
    def checkStateBool(self, index):
        return bool(self.checks[self.row_data[index.row()][0]])
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            return f"{self.row_data[row][0]:#x} {self.row_data[row][1]}"
    ...
    def update_table(self):
        ...
        self.controller.deci.info("Collecting Variable artifacts...")
        gvars = self.controller.deci.global_vars
        self.gvar_name_to_addr_map = {gvar.name: addr for addr, gvar in gvars.items()}
        for addr, gvar in gvars.items():
            self.data_dict[addr] = [addr, gvar.name]
            self.checks[addr] = False
            updated_row_keys.add(addr)
    ...
    def push(self):
        ...
        if self.proxymodel.rowCount() > 0:
            ...
            for i in range(self.proxymodel.rowCount()):
                ...
                if is_checked:
                    addrs_to_push.append(self.model.row_data[mappedIndex.row()][0])

        self.controller.force_push_global_vars(addrs_to_push)
...
class QGlobalsTable(QWidget):
    """Force-push tab listing global variables (addr-keyed)."""
    ...
    def _init_widgets(self):
        ...
        self.table = GlobalsTableView(self.controller, self.filteredit, stretch_col=1, col_count=2)
        ...
        self.push_button = QPushButton("Push 0 globals")
    ...
    def _update_push_button(self, *_):
        ...
        self.push_button.setText(f"Push {n} globals")
