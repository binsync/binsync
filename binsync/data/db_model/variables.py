...
class VariableUses(enum.Enum):
    """
    Various service states, VariableUses(status_value).name ==> VariableUses(VariableUses.RETURN_VALUE).name == "RETURN_VALUE"
    """
    RETURN_VALUE = 0
    ARGUMENT = 1
    LOCAL_VARIABLE = 2
...
class Variable(Base):
    ...
    fk_function_id = Column(String(128), ForeignKey(Binary.id), nullable=True)
    ...
    def __init__(self, address, size, used_as: VariableUses, fk_binary_id=None, fk_function_id=None):
...
class VariableInfo(Base):
    ...
    #function_name_index = Index('variable_function_name_index', 'name')

    def __init__(self, name, fk_variable_id, is_root, fk_user_id):
        self.name = name
