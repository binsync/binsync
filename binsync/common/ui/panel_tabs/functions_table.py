...
class FunctionTableModel(QAbstractTableModel):
    ...
    def __init__(self, controller: BinSyncController, data=None, parent=None):
        ...
        if data is None:
            self.row_data = []  # holds sublists of form: (addr, name, user_name, push_time)
        else:
            self.row_data = data
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            if index.column() == 0:
                return f"{self.row_data[index.row()][0]:#6x}"
            elif index.column() == 1:
            ...
            if len(self.row_data) != len(self.colordata):
                return None
...
class FunctionTableView(QTableView):
    ...
    def contextMenuEvent(self, event):
        ...
        if not (0 <= selected_row < len(self.model.row_data)) or not idx.isValid():
            valid_row = False
