...
class BinsyncPlugin(QObject, idaapi.plugin_t):
    ...
    def open_repo_selector(self):
        #self._repo_selector = RepoSelector(qApp.activeWindow())
        #self._repo_selector.show()
        self._repo_selector = RepoSelector()
        self._repo_selector.Compile()
        ok = self._repo_selector.Execute()
        if ok == 1:
            print("User Name: %s, Repo Dir: %s, Init Repo: %s" % (self._repo_selector.user_name, self._repo_selector.repo_dir, self._repo_selector.init_repo))
            print("DONE!")
        else:
            print("UH OH")
